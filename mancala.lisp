...
(defun genera-nodo (indice estado) ;indice es de donde saco mis canicas
	...
	(cond
		((<= cant-piedras-aux 13) ;Solamente con 13 o menos canicas podemos robar; de lo contario, seria imposible
			(setq recorrido (+ indice cant-piedras-aux))
			(cond
				((>= recorrido 14)
					(setq pos-final (+ 1 (mod recorrido 14))) ;Obtener la posicion donde termino.
				)
				(t
					(setq pos-final recorrido)
				)
			)
			(cond
				((and (<= pos-final 6) (or (= pos-final indice) (= (car (nthcdr (- pos-final 1) estado)) 0)))
					(rplaca (nthcdr 6 aux) (+ (car (nthcdr 6 aux)) 1 (car (nthcdr (- 13 pos-final) aux))))
					(rplaca (nthcdr (- pos-final 1) aux) 0)
					(rplaca (nthcdr (- 13 pos-final) aux) 0)
				)
			)
		)
	)
